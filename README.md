...
Each attribute is given their own individual components that controls the value. Value is passed through a character's context, each character has its own context to manage its own state.
...
Upon attribute update, current character attributes are checked against class requirements. UI has a visual update to indicate possible classes (class heading changes from light pink to light green).

3. When clicking on a class, display the minimum required statistics for that class

Uses a React-Bootstrap [Collapse](https://react-bootstrap.netlify.app/docs/utilities/transitions/#collapse) class to open and close a UI element that displays required attributes for class.
...
Implemented with Math.floor((attribute - 10) / 2)
...
Contents of SKILL_LIST are mapped to a React Bootstrap Table. Columns are the skill name, points allocated, attribute modifier, modifier value, and total skill value.
...
A summation of attributes is taken and compared to 70, disabling all incrementing buttons if it is greater.

8. Add the ability to edit multiple characters simultaneously with the same rules above

MainPage.js uses an array that contains all Character components, which can be added to or removed from. Each Character component is independent of each other, allowing for simultaneous control without interference.
